...
class SoftmaxCrossEntropyWithLogits():
  ...
  def forward(self, x, y):
    ...
    """
    # TODO
    return np.mean(np.log(np.sum(np.exp(x), axis=1) + 1e-13) - np.sum((x * y), axis=1))
  ...
  def backward_inputs(self, x, y):
    ...
    #print(softmax_x.shape)
    #print(y.shape)

    #print("softmax")
    return (softmax_x - y) / x.shape[0]
...
class L2Regularizer():
  ...
  def forward(self):
    ...
    """
    # TODO
    return 0.5 * self.weight_decay * np.square(np.linalg.norm(self.weights.ravel(), ord=2))
